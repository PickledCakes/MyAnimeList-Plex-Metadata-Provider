...
6. Add the provider URL to Plex:

```text
http://127.0.0.1:4567/tv
```

The installer registers the provider as a Windows Scheduled Task so it can start automatically whenever the server starts.
...
3. Refresh metadata for the affected series in Plex.
...
causes Plex to display the MAL score using Plex’s TMDB-style rating badge.

This setting does not convert the MAL score into a TMDB or IMDb score.
...
Reinstall or repair automatic startup:
...
A successful response confirms that the provider process is running. It does not necessarily confirm that Plex has registered the provider correctly or that every external metadata request is working.
...
The task should be configured to run at system startup.
...
The provider does not currently control all of Plex’s recommendation behaviour. A show appearing in that row does not necessarily mean that MAL identifies it as a sequel, prequel, spin-off, or adaptation.
...
- The provider currently targets TV/anime libraries rather than movie libraries.
